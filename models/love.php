...
class Love extends Db{
    ...
    function add($love_data){
        ...
        $sandwich_id = $this->data['sandwich_id'];
        $user_id = $_SESSION['user_logged_in'];

        // Check if already loved
        $sql = "SELECT * FROM loves WHERE sandwich_id = '$sandwich_id' AND user_id = '$user_id'";
        $love = $this->select($sql)[0];

        if( !empty($love['id']) ){ // already loved, delete the love
                $sql = "DELETE FROM loves WHERE sandwich_id = '$sandwich_id' AND user_id = '$user_id'";
                $this->execute($sql);
                $love_data['loved'] = 'unloved';
                $love_data['error'] = false;
        }else{
            $sql = "INSERT INTO loves (sandwich_id, user_id) VALUES('$sandwich_id', '$user_id')";
        ...
        $sql = "SELECT COUNT(id) AS love_count FROM loves WHERE sandwich_id = '$sandwich_id'";
